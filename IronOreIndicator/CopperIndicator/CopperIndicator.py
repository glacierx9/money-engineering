...
# coding=utf-8
"""
CopperIndicator - Short-Term Position Accumulation Strategy

SHORT-TERM TRADING STRATEGY with 7-Indicator Market Regime Detection:
- Triple EMA (12/24/45) - Faster EMAs for more reactive copper trading
- MACD (12/24/9) - Momentum confirmation & reversal detection
- RSI (12) - Slightly more sensitive mean reversion signals
...
- Other parameters same as IronOre (proven system)
"""
...
class CopperIndicator(pcts3.sv_object):
    """
    Copper Multi-Indicator Confirmation System

    Generates regime-aware buy/sell signals using multi-layer confirmation
    Tuned for SHFE Copper futures with faster parameters
    """
    ...
    def __init__(self):
        ...
        # Alpha parameters (smoothing factors) - FASTER for Copper
        self.alpha_12 = 2.0 / 13.0   # 12-period (same as Iron Ore)
        self.alpha_26 = 2.0 / 25.0   # 24-period (FASTER than Iron Ore's 26)
        self.alpha_50 = 2.0 / 46.0   # 45-period (FASTER than Iron Ore's 50)
        self.alpha_9 = 2.0 / 10.0    # 9-period (same)
        self.alpha_14 = 2.0 / 13.0   # 12-period RSI (FASTER than Iron Ore's 14)
        ...
        # Triple EMA states
        self.ema_12 = 0.0
        self.ema_26 = 0.0  # Actually 24-period due to alpha_26 change
        self.ema_50 = 0.0  # Actually 45-period due to alpha_50 change
    ...
    def _update_macd(self):
        ...
        # MACD line (already have EMA 12 and 24)
    ...
    def _update_rsi(self, close):
        ...
        # Update gain/loss EMAs (using 12-period alpha)
    ...
    def _calculate_signal_strength(self):
        ...
        # Factor 3: Trend alignment (EMA 12 vs 24)
    ...
    def _calculate_downtrend_reversal_score(self):
        ...
        # 1. EMA12 crossing above EMA24 (reversal signal)
        if self.ema_12 > self.ema_26:
            score += 1
...
async def on_init():
    ...
    if worker_no != 0 and metas and imports:
        ...
        logger.info("CopperIndicator initialized")
...
async def on_ready():
    ...
    logger.info("CopperIndicator ready")
